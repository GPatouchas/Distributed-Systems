        ...
        try {
            ...
            if (temp5 == null) {
                ...
                for (int i = 0; i < temp5.size(); i++) {
                    avg_distance += temp5.get(i).getTotalDistance();
                    avg_exercise_time += temp5.get(i).getTotalTime();
                    avg_elevation += temp5.get(i).getTotalelevation();
                }

                user_avg_exercise_time = avg_exercise_time / temp5.size();
                user_avg_distance = avg_distance / temp5.size();
                user_avg_elevation = avg_elevation / temp5.size();

                // System.out.println("User stats: ");
                // System.out.println("Average Exercise Time: " + user_avg_exercise_time + "
                // sec");
                // System.out.println("Average Distance Time: " + user_avg_distance + " km");
                // System.out.println("Average Elevation Time: " + user_avg_elevation + "
                // meters");

            }

            ArrayList<resultInfo> avg_stats = new ArrayList<>();
            ...
            avg_stats.add(user_avg);

            double sum_avg_exercise_time = 0;
            double sum_avg_distance = 0;
            double sum_avg_elevation = 0;
            ...
            for (int j = 0; j < 3; j++) {
                if (j == 0 && h.get("user1") != null) {
                    for (int z = 0; z < h.get("user1").size(); z++) {
                        sum_avg_distance += h.get("user1").get(z).getTotalDistance();
                        sum_avg_exercise_time += h.get("user1").get(z).getTotalTime();
                        sum_avg_elevation += h.get("user1").get(z).getTotalelevation();
                    }
                } else if (j == 1 && h.get("user2") != null) {
                    for (int z = 0; z < h.get("user2").size(); z++) {
                        sum_avg_distance += h.get("user2").get(z).getTotalDistance();
                        sum_avg_exercise_time += h.get("user2").get(z).getTotalTime();
                        sum_avg_elevation += h.get("user2").get(z).getTotalelevation();
                    }
                } else if (j == 2 && h.get("user3") != null) {
                    for (int z = 0; z < h.get("user3").size(); z++) {
                        sum_avg_distance += h.get("user3").get(z).getTotalDistance();
                        sum_avg_exercise_time += h.get("user3").get(z).getTotalTime();
                        sum_avg_elevation += h.get("user3").get(z).getTotalelevation();
                    }
                }
            }
            ...
            sum_avg_exercise_time = sum_avg_exercise_time / (num);
            sum_avg_distance = sum_avg_distance / (num);
            sum_avg_elevation = sum_avg_elevation / (num);

            // System.out.println("Stats from all users combined: \n");
            // System.out.println("Average Exercise Time: " + sum_avg_exercise_time + "
            // sec");
            // System.out.println("Average Distance: " + sum_avg_distance + " km");
            // System.out.println("Average Elevation: " + sum_avg_elevation + " meters");

            resultInfo all_users_avg = new resultInfo();
            all_users_avg.setTotalDistance(sum_avg_distance);
            all_users_avg.setTotalTime(sum_avg_exercise_time);
            all_users_avg.setTotalelevation(sum_avg_elevation);

            avg_stats.add(all_users_avg);

            outputstream.writeObject(avg_stats);
